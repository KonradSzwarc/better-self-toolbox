...
export function LanguageSwitcherReact({ label, value, languages, className }: Props) {
  return (
    <DropdownMenu.Root modal={false}>
      ...
      <DropdownMenu.Portal>
        <DropdownMenu.Content align="end" sideOffset={8} alignOffset={-16} className="grid border bg-white shadow-sm">
          {Object.values(languages).map(({ flag, label, path }, i, arr) => (
            <>
              <DropdownMenu.Item asChild className="block min-w-20 p-2 whitespace-nowrap text-zinc-950">
                <a href={path}>
                  {flag} {label}
                </a>
              </DropdownMenu.Item>
              {i < arr.length - 1 && <DropdownMenu.Separator className="h-px bg-zinc-950" />}
            </>
          ))}
